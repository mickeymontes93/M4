...
#define MAXIC     10000  //tamaño máximo del array de código-p
...
typedef struct {
   ...
   //  int     di; //dirección o desplazamiento (0..32767)
     ...
     int ti; // indica el tipo de dato con el que la instruccion va a trabajar:
                  /*
...
int main(int argc,char *argv[]) {
 if (argc!=2) //analizar argumentos de main
    ...
    if ( (obj=fopen(argv[1],"r+b"))==NULL ) 
       ...
       ic=0;
       printf("IC----->%d",ic);
       fread(&codigo[0],sizeof(codigo_intermedio),1,obj);
       printf("IC----->%d",codigo[ic].ti);
      // printf("\n----tam: %d  -  linea: %s ----\n",sizeof(codigo_intermedio),codigo[ic].Dato.cadena);
       while ( !feof(obj) ) {
             ++ic;
