...
#define LONGSTACK 1000  /*Longitud del stack de datos                    */
...
int main(int argc,char *argv[]) {
	if (argc!=2) //analizar argumentos de main
	 ...
	 else if ( (obj=fopen(argv[1],"r+b"))==NULL ) {
	//    if ( (obj=fopen("ejemplo1.ob","r+b"))==NULL ) {
	  ...
	  // printf("\n IC----->%d",ic);
	   ...
	   //printf("\tMNEMONICO--->", mnemonico[codigo[ic].f]);
	   printf("\tF----->%d",codigo[ic].f);
	   printf("\tN----->%d",codigo[ic].ni);
	   printf("\tD----->%d",codigo[ic].Dato.entero);
	   printf("\n");
		...
		switch(i.f) {
            ...
            case OPR: // OPR nivel tipo_opr tipo_dato
				switch(i.Dato.entero) { 
                    ...
                    case 2: //----------> SUMA
                        --s;
                        if(p[s].t==TIPO_ENTERO && p[s+1].t==TIPO_ENTERO){
	                          ...
	                          p[s].t=TIPO_ENTERO;
                        }else if(p[s].t==TIPO_ENTERO && p[s+1].t==TIPO_FLOAT){
	                          printf("\n[ s = %d ] [ OPR ---> N = %d D= %d T = %d ] suma de %d + %d",s,i.ni,i.Dato.entero,i.t,p[s].Dato.entero,p[s+1].Dato.entero);
	                          p[s].Dato.flotante=p[s].Dato.entero+p[s+1].Dato.flotante;
	                          p[s].t=TIPO_FLOAT;
                        }else if(p[s].t==TIPO_FLOAT && p[s+1].t==TIPO_ENTERO){
	                          printf("\n[ s = %d ] [ OPR ---> N = %d D= %d T = %d ] suma de %d + %d",s,i.ni,i.Dato.entero,i.t,p[s].Dato.entero,p[s+1].Dato.entero);
	                          p[s].Dato.flotante=p[s].Dato.flotante+p[s+1].Dato.entero;
	                          p[s].t=TIPO_FLOAT;
                        }else if(p[s].t==TIPO_FLOAT && p[s+1].t==TIPO_FLOAT){
	                          printf("\n[ s = %d ] [ OPR ---> N = %d D= %d T = %d ] suma de %d + %d",s,i.ni,i.Dato.entero,i.t,p[s].Dato.entero,p[s+1].Dato.entero);
                    ...
                    case 5: //---------->  DIVISION
						--s;
						if((p[s+1].t==TIPO_ENTERO && p[s+1].Dato.entero==0) or (p[s+1].t==TIPO_FLOAT && p[s+1].Dato.flotante==0)) {
						    fclose(obj);
                    ...
                    case 10: //----------> MENOR QUE
                        --s;
                        if(p[s].t==TIPO_ENTERO) op1=float(p[s].Dato.entero);
                        else op1=p[s].Dato.flotante;

                        if(p[s+1].t==TIPO_ENTERO) op2=float(p[s+1].Dato.entero);
                   ...
                   //-------------------- FUNCIONES DE M4 -------------------------     
                    ...
                    case 40: //----------> EQUAL
                    	--s;
                    	resp= strcmp(p[s].Dato.cadena,p[s+1].Dato.cadena);
                    	if(resp<0 || resp > 0){
                    		p[s].Dato.entero=0;
                    	}else{
                    		p[s].Dato.entero=1;
                    ...
                    case 42: //-------------> OR
                    	--s;
                    	if(p[s].Dato.entero == 0 && p[s+1].Dato.entero==0) p[s].Dato.entero=0;
                    	if(p[s].Dato.entero == 0 && p[s+1].Dato.entero==1) p[s].Dato.entero=1;
                    	if(p[s].Dato.entero == 1 && p[s+1].Dato.entero==0) p[s].Dato.entero=1;
                    	if(p[s].Dato.entero == 1 && p[s+1].Dato.entero==1) p[s].Dato.entero=1;
                    	break;
                    case 43: //-------------> AND
                    	--s;
                    	if(p[s].Dato.entero == 0 && p[s+1].Dato.entero==0) p[s].Dato.entero=0;
                    	if(p[s].Dato.entero == 0 && p[s+1].Dato.entero==1) p[s].Dato.entero=0;
                    	if(p[s].Dato.entero == 1 && p[s+1].Dato.entero==0) p[s].Dato.entero=0;
                    	if(p[s].Dato.entero == 1 && p[s+1].Dato.entero==1) p[s].Dato.entero=1;
                    	break;
...
void substring_cad(int s){
	...
	//p[s+2] ---> fin
	s=s-2;
	char string[MAXSTRING],respuesta[MAXSTRING];
	int inicio, fin;
